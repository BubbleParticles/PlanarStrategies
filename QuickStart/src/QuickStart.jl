...
function call!(s::SC, ::ResetStrategy)
    ...
    attrs[:hl] = Dict(
        ii => (Ref(DateTime(0)), oti.WMA{DFT}(; period=120)) for ii in s.universe
    )
    # attrs[:slope_window] = 180
    attrs[:extremas] = Dict(ii => stt.MovingExtrema(60) for ii in s.universe)
    attrs[:uni_iter] = ([s.universe...], Ref(DateTime(0)))
    attrs[:backoff] = Dict(ii => DateTime(0) for ii in s.universe)
    attrs[:lsr] = LongShortRatio()
    kama_kwargs = (; period=60, slow_ema_constant_period=15, fast_ema_constant_period=120)
    attrs[:buysigs] = Dict(ii => oti.KAMA{DFT}(; kama_kwargs...) for ii in s.universe)
    attrs[:sellsigs] = Dict(ii => oti.KAMA{DFT}(; kama_kwargs...) for ii in s.universe)
    attrs[:pnl_n] = 21
...
function iswarm(s::RTStrategy, ii)
    getfield(s, :config)[:warmup][ii]
end

function iswarm(s::SimStrategy, ii)
    true
end

function dowarmup(s, ii=nothing, ats=nothing)
    if islive(s)
        if !isnothing(ii)
            call!(copypnl!, s, ii, ats, SimWarmup(); n_candles=s.warmup_candles)
        else
            ats = @something ats available(s.timeframe, now())
            n_candles = s.warmup_candles
            for ii in universe(s)
                call!(copypnl!, s, ii, ats, SimWarmup(); n_candles)
            end
        end
    end
end

function trackhl!(s, ii, ats)
    data = ohlcv(ii)
...
function poll(s, ts)
    ...
    if last_sort[] < ats
        extremas = s.extremas
        sort!(uni_iter; by=ii -> mmvol(s, ii, ats, extremas))
        last_sort[] = ats
    end

    liveloop(s, uni_iter) do ii
        dowarmup(s, ii, ats)
        ...
        if !iswarm(s, ii) ||
            stt.isstaleohlcv(s, ii; ats, tf, backoff) ||
            stt.isstalesignal(s, ats; lifetime)
            # @ldebug 3 "quickstart: stale"
            return nothing
        end
        # pnl at time `ats`
        trackpnl!(s, ii, ats, ts; interval=pnl_interval)
        ...
        # if !istrending!(s, ii, ats, :sma) ||
        #     !istrending!(s, ii, ats, :vtx) ||
        #     !istrending!(s, ii, ats, :roc)
        #     @ldebug 1 "quickstart: stop hit (or trend fail)" ii
        #     return nothing
        # end
        ...
        buy = isbuy(s, ii, ats)
        sell = issell(s, ii, ats)
        oti.fit!(buysigs[ii], DFT(buy))
        oti.fit!(sellsigs[ii], DFT(sell))

        if stt.isrecenttrade(ii, ats, tf; cd=cooldown)
            return nothing
        end

        thisbuy = @coalesce buysigs[ii].value 0.0
        thissell = @coalesce sellsigs[ii].value 0.0
        ratio = thisbuy / thissell
        if ratio > s[:buy_trigger]
            @ldebug 2 "quickstart: isbuy" ii
            buyorsell!(s, ii, ats, ts, Buy)
        elseif ratio < s[:sell_trigger]
            @ldebug 2 "quickstart: issell" ii
            buyorsell!(s, ii, ats, ts, Sell)
        end
        @ldebug 2 "quickstart: nosignal" ii
        if s.ismake
            ensure_market_make(s, ii, ts; ats)
        end
    end
end

function mmvol(s, ii, ats, exts)
    obj = exts[ii]
    ov = ohlcv(ii)
...
function calculate_position_adjustment(s::SC, ii, ats)
    ...
    #     # atr_sig = strategy_signal(s, ii, :my_atr)
    #     # trend_sig = strategy_signal(s, ii, :my_trend)
    ...
    @ldebug 2 "position adjustment calc" ii final_mult
    return final_mult
end

function get_target_position_size(s::SC, ii, ps::PositionSide, ats)
    ...
    c = let pos = position(ii, ps)
        ai_coll = collateral(pos)
        avl_stake = tot * min(s.qt[ii], s.qt_base) - ai_coll
...
function call!(s::SC, ii, trade::Trade, ::NewTrade)
    @ldebug 2 "quickstart: new trade func" ii trade
    lsr = s.lsr
    lsr.long = 0
    lsr.short = 0
    for ii in s.universe
        pside = islong(posside(ii)) ? :long : :short
...
function trade_amount(s, ii, ats, ps::PositionSide)
    @ldebug 2 "quickstart: trade amount" ii ps
    lev = getlev(s, ii)
    if lev < one(lev)
        return ii.limits.amount.min
    end
    ...
    if is_position_open
        ...
        @ldebug 2 "using target position size" ii target_size c
    end

    @ldebug 2 "quickstart: trade amount" ii c
    c = let pos = position(ii, ps)
        ai_coll = collateral(pos)
        ...
        # Note: VTX signal reference removed - users can add their own signal logic here
        plus_rate = 1.0  # Default rate, users can customize based on their signals
        @ldebug 2 "quickstart: trade amount" ii avl_stake
        # The cash should not exceed the maximum available stake for the asset
        min(avl_stake * plus_rate, c * drawdown^13)
    end
    # If the cash is less than 1$, do not trade
    c <= zero(c) && return ii.limits.amount.min
    price = closeat(ii, ats)
    amt = abs(c / price)
    amt / lev
end

function calculate_trade_amount(s::SC, ii, ats, ts, side, pos_side)
    # Check if position is already open
    is_position_open = isopen(ii, pos_side)

    if is_position_open
        # Position is open - calculate target size and adjust accordingly
        current_pos = position(ii, pos_side)
        current_size = abs(value(cash(current_pos)))
        target_size = get_target_position_size(s, ii, pos_side, ats)

        # Calculate the adjustment needed with threshold
        size_diff = abs(target_size - current_size)
        size_threshold = max(current_size * 0.1, ii.limits.amount.min)  # 10% of current size or min amount

        if size_diff > size_threshold
            if target_size > current_size
                # Need to increase position
                amount = target_size - current_size
                @ldebug 2 "increasing position size" ii current_size target_size amount size_threshold
            else
                # Need to decrease position
                amount = current_size - target_size
                @ldebug 2 "decreasing position size" ii current_size target_size amount size_threshold
                # If the new target size is at or below min, close the position
                if target_size <= ii.limits.amount.min
                    @ldebug 1 "closing position due to reduction below min" ii current_size target_size amount min_amount =
                        ii.limits.amount.min
                    closeposition!(s, ii, ts; pside=pos_side)
            ...
            amount = 0.0
            @ldebug 2 "no position adjustment needed" ii current_size target_size size_diff size_threshold
        end
    else
        # Position is closed - use trade_amount for new position
        amount = trade_amount(s, ii, ats, pos_side)
    end

    return amount
end

function buyorsell!(s::SC, ii, ats, ts, side)
    @ldebug 2 "quickstart: buyorsell" ii = raw(ii) side
    trade_pos_side = ifelse(side == Buy, Long(), Short())
    @deassert ii.asset.qc == nameof(s.cash)
    p = @something position(ii) position(ii, trade_pos_side)
    amount = 0.0

    if posside(p) == trade_pos_side
        @ldebug 2 "same side" raw(ii) amount
        if hasentrycash(s, ii) && belowtotal(s, ii, p)
            amount = trade_amount(s, ii, ats, trade_pos_side)
            # amount = calculate_trade_amount(s, ii, ats, ts, side, trade_pos_side)

            if amount isa Number && amount > 0.0
                trade!(s, ii, ats, ts; pos=trade_pos_side, side, amount)::TradeResult
            end
        end
    else
        @ldebug 2 "oppo side" raw(ii) amount
        open_pos_side = opposite(trade_pos_side)
        amount = abs(freecash(ii, open_pos_side))
        if amount > 0.0
            trade!(s, ii, ats, ts; pos=open_pos_side, side, amount)::TradeResult
        end
        # amount = calculate_trade_amount(s, ii, ats, ts, side, trade_pos_side)
        amount = trade_amount(s, ii, ats, trade_pos_side)
        trade!(s, ii, ats, ts; pos=trade_pos_side, side, amount)::TradeResult
