...
"""
    trade!(s::SC, ai::AssetInstance, ats::DateTime, ts::DateTime; 
...
- `ai::AssetInstance`: Asset instance to trade
...
"""
function trade!(s::SC, ai::AssetInstance, ats::DateTime, ts::DateTime; 
                pos::PositionSide, side::PositionSide, amount::Float64, kwargs...)
    try
        @debug "Initiating trade" ai=ai pos=pos side=side amount=amount
        
        # Validate trade parameters
        if !validate_trade_parameters(s, ai, pos, side, amount)
            @warn "Trade validation failed" ai=ai pos=pos side=side amount=amount
            return false
        end
        
        # Check trade cooldown
        if !check_trade_cooldown(s, ai, ts)
            @debug "Trade blocked by cooldown" ai=ai
            return false
        end
        
        # Calculate final trade amount with adjustments
        final_amount = calculate_final_trade_amount(s, ai, ats, side, amount)
        
        if final_amount <= 0
            @debug "Final trade amount is zero or negative" ai=ai original_amount=amount final_amount=final_amount
            return false
        end
        
        # Select optimal order type
        order_type = select_order_type(s, ai, side, final_amount, kwargs)
        
        # Calculate order price
        order_price = calculate_order_price(s, ai, side, order_type)
        
        # Apply leverage adjustments
        leverage_info = calculate_leverage_adjustment(s, ai, side, final_amount)
        
        # Normalize order parameters
        normalized_amount = normalize_order_amount(s, ai, final_amount)
        normalized_price = normalize_order_price(s, ai, order_price)
        
        # Create order parameters
        order_params = create_order_parameters(s, ai, side, normalized_amount, normalized_price, 
                                             order_type, leverage_info, kwargs)
        
        # Execute the order
        order_result = execute_order(s, ai, order_params)
        
        if order_result.success
            # Update tracking and state
            update_trade_tracking(s, ai, ts, order_result)
            @info "Trade executed successfully" ai=ai order_id=order_result.order_id amount=normalized_amount price=normalized_price
            return true
        else
            # Handle order failure
            handle_order_failure(s, ai, order_params, order_result.error)
            return false
        end
        
    catch e
        @error "Error in trade execution" ai=ai pos=pos side=side amount=amount error=e
        return false
    end
end

"""
    validate_trade_parameters(s::SC, ai::AssetInstance, pos::PositionSide, side::PositionSide, amount::Float64)

Validate trade parameters before execution.
"""
function validate_trade_parameters(s::SC, ai::AssetInstance, pos::PositionSide, side::PositionSide, amount::Float64)
    try
        # Check amount is positive
        if amount <= 0
            @warn "Invalid trade amount" ai=ai amount=amount
            return false
        end
        
        # Check minimum trade amount
        min_amount = get_min_position_size(s, ai)
        if amount < min_amount
            @debug "Trade amount below minimum" ai=ai amount=amount min_amount=min_amount
            return false
        end
        ...
        if side isa Long && current_position_value + amount > max_position
            @warn "Trade would exceed maximum position size" ai=ai current=current_position_value amount=amount max=max_position
            return false
        end
        
        # Check available cash for increasing positions
        if is_increasing_position(s, ai, side)
            ...
            if amount > usable_cash
                @warn "Insufficient cash for trade" ai=ai amount=amount available=usable_cash
                return false
            end
        end
        
        # Check market hours and trading status
        if !is_market_open(s, ai, now())
        ...
        @error "Error validating trade parameters" ai=ai error=e
        return false
    end
end

"""
    check_trade_cooldown(s::SC, ai::AssetInstance, ts::DateTime)

Check if sufficient time has passed since last trade.
"""
function check_trade_cooldown(s::SC, ai::AssetInstance, ts::DateTime)
    try
        cooldown_period = get(s.config, :trade_cooldown, Minute(1))
        
        if !haskey(s.config, :last_trade_time)
            s.config[:last_trade_time] = Dict{AssetInstance, DateTime}()
        end
        
        last_trade_times = s.config[:last_trade_time]
        
        if haskey(last_trade_times, ai)
            time_since_last = ts - last_trade_times[ai]
            if time_since_last < cooldown_period
                @debug "Trade cooldown active" ai=ai time_remaining=(cooldown_period - time_since_last)
                return false
            end
        end
        ...
        @error "Error checking trade cooldown" ai=ai error=e
        return false
    end
end

"""
    calculate_final_trade_amount(s::SC, ai::AssetInstance, ats::DateTime, side::PositionSide, amount::Float64)

Calculate final trade amount with all adjustments applied.
"""
function calculate_final_trade_amount(s::SC, ai::AssetInstance, ats::DateTime, side::PositionSide, amount::Float64)
    try
        ...
        volatility_multiplier = calculate_volatility_adjustment(s, ai)
        final_amount *= volatility_multiplier
        
        # Ensure final amount is within bounds
        min_amount = get_min_position_size(s, ai)
        max_amount = get_max_single_trade_amount(s, ai)
        
        final_amount = clamp(final_amount, min_amount, max_amount)
        
        @debug "Trade amount calculation" ai=ai original=amount final=final_amount adjustment=adjustment_multiplier risk=risk_multiplier drawdown=drawdown_multiplier volatility=volatility_multiplier
        
        return final_amount
        
    catch e
        @error "Error calculating final trade amount" ai=ai amount=amount error=e
        return 0.0
    end
end

"""
    select_order_type(s::SC, ai::AssetInstance, side::PositionSide, amount::Float64, kwargs)

Select optimal order type based on market conditions and strategy configuration.
"""
function select_order_type(s::SC, ai::AssetInstance, side::PositionSide, amount::Float64, kwargs)
    try
        ...
        market_conditions = analyze_market_conditions(s, ai)
        ...
        @error "Error selecting order type" ai=ai error=e
        return :gtc  # Safe fallback
    end
end

"""
    calculate_order_price(s::SC, ai::AssetInstance, side::PositionSide, order_type::Symbol)

Calculate optimal order price based on order type and market conditions.
"""
function calculate_order_price(s::SC, ai::AssetInstance, side::PositionSide, order_type::Symbol)
    try
        current_price = last(s.universe[ai].close)
        ...
        spread_info = get_spread_info(s, ai)
        ...
        @error "Error calculating order price" ai=ai side=side order_type=order_type error=e
        return last(s.universe[ai].close)  # Fallback to current price
    end
end

"""
    calculate_leverage_adjustment(s::SC, ai::AssetInstance, side::PositionSide, amount::Float64)

Calculate leverage adjustments for new and existing positions.
"""
function calculate_leverage_adjustment(s::SC, ai::AssetInstance, side::PositionSide, amount::Float64)
    try
        ...
        has_existing_position = haspositions(s, ai)
        current_position = has_existing_position ? position(s, ai) : 0.0
        ...
        risk_adjustment = calculate_risk_leverage_adjustment(s, ai)
        ...
        @error "Error calculating leverage adjustment" ai=ai side=side amount=amount error=e
        return (
            leverage = 1.0,
...
"""
    normalize_order_amount(s::SC, ai::AssetInstance, amount::Float64)

Normalize order amount to exchange requirements.
"""
function normalize_order_amount(s::SC, ai::AssetInstance, amount::Float64)
    try
        current_price = last(s.universe[ai].close)
        ...
        @error "Error normalizing order amount" ai=ai amount=amount error=e
        return 0.0
    end
end

"""
    normalize_order_price(s::SC, ai::AssetInstance, price::Float64)

Normalize order price to exchange tick size requirements.
"""
function normalize_order_price(s::SC, ai::AssetInstance, price::Float64)
    try
        tick_size = get_tick_size(s, ai)
        return round(price / tick_size) * tick_size
    catch e
        @error "Error normalizing order price" ai=ai price=price error=e
        return price
    end
end

"""
    create_order_parameters(s::SC, ai::AssetInstance, side::PositionSide, amount::Float64, 
                           price::Float64, order_type::Symbol, leverage_info, kwargs)

Create comprehensive order parameters for execution.
"""
function create_order_parameters(s::SC, ai::AssetInstance, side::PositionSide, amount::Float64, 
                                price::Float64, order_type::Symbol, leverage_info, kwargs)
    try
        ...
        @error "Error creating order parameters" ai=ai side=side amount=amount error=e
        return Dict{Symbol, Any}()
    end
end

"""
    execute_order(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any})

Execute order with proper error handling and result tracking.
"""
function execute_order(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any})
    try
        # Generate unique order ID
        order_id = generate_order_id()
        order_params[:order_id] = order_id
        
        @info "Executing order" ai=ai order_id=order_id type=order_params[:type] amount=order_params[:amount] price=order_params[:price]
        ...
        if rand() < success_probability
            ...
            if order_params[:type] == :market
                ...
                execution_price = last(s.universe[ai].close) * (1.0 + slippage)
            end
        ...
        @error "Error executing order" ai=ai order_params=order_params error=e
        return (
            success = false,
...
"""
    update_trade_tracking(s::SC, ai::AssetInstance, ts::DateTime, order_result)

Update trade tracking and strategy state after successful order execution.
"""
function update_trade_tracking(s::SC, ai::AssetInstance, ts::DateTime, order_result)
    try
        # Update last trade time
        if !haskey(s.config, :last_trade_time)
            s.config[:last_trade_time] = Dict{AssetInstance, DateTime}()
        end
        s.config[:last_trade_time][ai] = ts
        ...
        if !haskey(s.config, :trade_history)
            s.config[:trade_history] = Dict{AssetInstance, Vector{NamedTuple}}()
        end
        
        if !haskey(s.config[:trade_history], ai)
            s.config[:trade_history][ai] = NamedTuple[]
        end
        ...
        push!(s.config[:trade_history][ai], trade_record)
        
        # Keep only recent trade history (last 1000 trades per asset)
        if length(s.config[:trade_history][ai]) > 1000
            deleteat!(s.config[:trade_history][ai], 1:100)
        end
        
        @debug "Trade tracking updated" ai=ai order_id=order_result.order_id total_trades=s.config[:total_trades]
        
    catch e
        @error "Error updating trade tracking" ai=ai order_result=order_result error=e
    end
end
...
"""
    is_increasing_position(s::SC, ai::AssetInstance, side::PositionSide)

Check if the trade would increase the current position size.
"""
function is_increasing_position(s::SC, ai::AssetInstance, side::PositionSide)
    if !haspositions(s, ai)
...
"""
function analyze_market_conditions(s::SC, ai::AssetInstance)
    try
        ohlcv = s.universe[ai]
        ...
        @error "Error analyzing market conditions" ai=ai error=e
        return (volatility = 0.02, spread_pct = 0.001, volume_ratio = 1.0)
    end
end

"""
    calculate_volatility_adjustment(s::SC, ai::AssetInstance)

Calculate position size adjustment based on current volatility.
"""
function calculate_volatility_adjustment(s::SC, ai::AssetInstance)
    try
        market_conditions = analyze_market_conditions(s, ai)
        ...
        @error "Error calculating volatility adjustment" ai=ai error=e
        return 1.0
    end
end

"""
    get_max_single_trade_amount(s::SC, ai::AssetInstance)

Get maximum amount for a single trade.
"""
function get_max_single_trade_amount(s::SC, ai::AssetInstance)
    available_cash = cash(s, ai)
...
"""
    get_spread_info(s::SC, ai::AssetInstance)

Get bid-ask spread information for an asset.
"""
function get_spread_info(s::SC, ai::AssetInstance)
...
"""
    calculate_risk_leverage_adjustment(s::SC, ai::AssetInstance)

Adjust leverage based on current risk exposure.
"""
function calculate_risk_leverage_adjustment(s::SC, ai::AssetInstance)
    try
        ...
        @error "Error calculating risk leverage adjustment" ai=ai error=e
...
"""
function handle_order_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)
    try
        @warn "Handling order error" ai=ai order_id=get(order_params, :order_id, "unknown") error=error
        ...
        if success
            @info "Order error handled successfully" ai=ai error_category=error_category
        else
            @error "Failed to handle order error" ai=ai error_category=error_category
            
            # Send notification for unhandled errors
            send_error_notification(s, ai, error, order_params)
        end
        
        return success
        
    catch e
        @error "Error in order error handling" ai=ai error=e original_error=error
        return false
    end
end
...
"""
    handle_margin_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)

Handle insufficient margin errors by reducing leverage or position size.
"""
function handle_margin_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)
    try
        @info "Handling margin error with leverage reduction" ai=ai
        ...
        @info "Retrying order with reduced leverage" ai=ai original_leverage=current_leverage new_leverage=new_leverage
        
        # Execute fallback order
        result = execute_order(s, ai, fallback_params)
        
        if result.success
            @info "Margin error resolved with leverage reduction" ai=ai order_id=result.order_id
            update_trade_tracking(s, ai, now(), result)
            return true
        else
            @warn "Fallback order also failed" ai=ai error=result.error
            return false
        end
        
    catch e
        @error "Error handling margin error" ai=ai error=e
        return false
    end
end

"""
    handle_price_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)

Handle price out of range errors by adjusting price or switching to market order.
"""
function handle_price_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)
    try
        @info "Handling price error with market order fallback" ai=ai
        ...
        @info "Switching to market order" ai=ai original_type=order_params[:type]
        
        # Execute market order
        result = execute_order(s, ai, fallback_params)
        
        if result.success
            @info "Price error resolved with market order" ai=ai order_id=result.order_id
            update_trade_tracking(s, ai, now(), result)
            return true
        else
            @warn "Market order fallback also failed" ai=ai error=result.error
            return false
        end
        
    catch e
        @error "Error handling price error" ai=ai error=e
        return false
    end
end

"""
    handle_market_closed_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)

Handle market closed errors by scheduling order for later execution.
"""
function handle_market_closed_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)
    try
        @info "Market closed, scheduling order for later" ai=ai
        
        # Add to pending orders queue
        if !haskey(s.config, :pending_orders)
            s.config[:pending_orders] = Dict{AssetInstance, Vector{Dict{Symbol, Any}}}()
        end
        
        if !haskey(s.config[:pending_orders], ai)
            s.config[:pending_orders][ai] = Dict{Symbol, Any}[]
        end
        ...
        push!(s.config[:pending_orders][ai], pending_order)
        
        @info "Order scheduled for retry when market opens" ai=ai order_id=get(order_params, :order_id, "unknown")
        
        return true  # Successfully scheduled
        
    catch e
        @error "Error handling market closed error" ai=ai error=e
        return false
    end
end

"""
    handle_network_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)

Handle network timeout errors with exponential backoff retry.
"""
function handle_network_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)
    try
        retry_count = get(order_params, :retry_count, 0)
        max_retries = get(s.config, :max_network_retries, 3)
        
        if retry_count >= max_retries
            @error "Maximum network retries exceeded" ai=ai retry_count=retry_count
            return false
        end
        ...
        @info "Network error, retrying with backoff" ai=ai retry_count=retry_count delay=delay
        ...
        result = execute_order(s, ai, retry_params)
        
        if result.success
            @info "Network error resolved on retry" ai=ai order_id=result.order_id retry_count=retry_count + 1
            update_trade_tracking(s, ai, now(), result)
            return true
        else
            # Recursive retry if still failing
            return handle_network_error(s, ai, retry_params, result.error)
        end
        
    catch e
        @error "Error handling network error" ai=ai error=e
        return false
    end
end

"""
    handle_rejection_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)

Handle order rejection errors by analyzing and adjusting parameters.
"""
function handle_rejection_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)
    try
        @info "Handling order rejection" ai=ai error=error
        ...
        if contains(lowercase(error), "size") || contains(lowercase(error), "amount")
            ...
            if new_amount < min_amount
                @warn "Adjusted amount below minimum, canceling order" ai=ai new_amount=new_amount min_amount=min_amount
                return false
            end
            ...
            if result.success
                @info "Rejection resolved with size adjustment" ai=ai order_id=result.order_id
                update_trade_tracking(s, ai, now(), result)
                return true
            end
        end
        ...
        return handle_price_error(s, ai, order_params, error)
        
    catch e
        @error "Error handling rejection error" ai=ai error=e
        return false
    end
end

"""
    handle_balance_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)

Handle insufficient balance errors by adjusting order size to available funds.
"""
function handle_balance_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)
    try
        @info "Handling balance error" ai=ai
        ...
        if usable_cash <= 0
            @warn "No usable cash available" ai=ai available_cash=available_cash reserve_amount=reserve_amount
            return false
        end
        ...
        if adjusted_amount < min_amount
            @warn "Adjusted amount below minimum after balance check" ai=ai adjusted_amount=adjusted_amount min_amount=min_amount
            return false
        end
        ...
        @info "Retrying with adjusted amount" ai=ai original_amount=order_params[:amount] adjusted_amount=adjusted_amount
        
        result = execute_order(s, ai, fallback_params)
        
        if result.success
            @info "Balance error resolved with amount adjustment" ai=ai order_id=result.order_id
            update_trade_tracking(s, ai, now(), result)
            return true
        else
            @warn "Adjusted order also failed" ai=ai error=result.error
            return false
        end
        
    catch e
        @error "Error handling balance error" ai=ai error=e
        return false
    end
end

"""
    handle_position_limit_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)

Handle position limit errors by closing existing positions or reducing order size.
"""
function handle_position_limit_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)
    try
        @info "Handling position limit error" ai=ai
        ...
        if haspositions(s, ai)
            ...
            if (current_position > 0 && order_side isa Long) || (current_position < 0 && order_side isa Short)
                @info "Attempting partial position close to make room" ai=ai current_position=current_position
                ...
                close_params = Dict{Symbol, Any}(
                    :asset => ai,
                    :side => close_side,
                    :amount => close_amount * last(s.universe[ai].close),  # Convert to base currency
                    :type => :market,  # Use market order for quick execution
                    :reason => "position_limit_management"
                )
                
                close_result = execute_order(s, ai, close_params)
                
                if close_result.success
                    @info "Partial position closed, retrying original order" ai=ai
                    
                    # Wait a moment for position update
                    sleep(1.0)
                    
                    # Retry original order
                    result = execute_order(s, ai, order_params)
                    
                    if result.success
                        @info "Position limit resolved with partial close" ai=ai order_id=result.order_id
                        update_trade_tracking(s, ai, now(), result)
                        return true
                    end
                end
        ...
        if reduced_amount < min_amount
            @warn "Cannot reduce order size further" ai=ai reduced_amount=reduced_amount min_amount=min_amount
            return false
        end
        ...
        if result.success
            @info "Position limit resolved with size reduction" ai=ai order_id=result.order_id
            update_trade_tracking(s, ai, now(), result)
            return true
        else
            return false
        ...
        @error "Error handling position limit error" ai=ai error=e
        return false
    end
end

"""
    handle_parameter_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)

Handle invalid parameter errors by correcting parameters and retrying.
"""
function handle_parameter_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)
    try
        @info "Handling parameter error" ai=ai error=error
        ...
        if haskey(corrected_params, :amount)
            corrected_params[:amount] = normalize_order_amount(s, ai, corrected_params[:amount])
        end
        
        if haskey(corrected_params, :price) && corrected_params[:price] !== nothing
            corrected_params[:price] = normalize_order_price(s, ai, corrected_params[:price])
        end
        ...
        @info "Retrying with corrected parameters" ai=ai
        
        result = execute_order(s, ai, corrected_params)
        
        if result.success
            @info "Parameter error resolved with corrections" ai=ai order_id=result.order_id
            update_trade_tracking(s, ai, now(), result)
            return true
        else
            @warn "Corrected order also failed" ai=ai error=result.error
            return false
        end
        
    catch e
        @error "Error handling parameter error" ai=ai error=e
        return false
    end
end

"""
    handle_unknown_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)

Handle unknown errors with generic fallback strategies.
"""
function handle_unknown_error(s::SC, ai::AssetInstance, order_params::Dict{Symbol, Any}, error::String)
    try
        @warn "Handling unknown error with market order fallback" ai=ai error=error
        ...
        if result.success
            @info "Unknown error resolved with market order" ai=ai order_id=result.order_id
            update_trade_tracking(s, ai, now(), result)
            return true
        else
            @error "All fallback strategies failed" ai=ai final_error=result.error
            return false
        end
        
    catch e
        @error "Error handling unknown error" ai=ai error=e
        return false
    end
end

"""
    update_error_tracking(s::SC, ai::AssetInstance, error_category::Symbol, error::String)

Update error tracking statistics for monitoring and analysis.
"""
function update_error_tracking(s::SC, ai::AssetInstance, error_category::Symbol, error::String)
    try
        ...
        if !haskey(s.config, :error_history)
            s.config[:error_history] = Tuple{DateTime, AssetInstance, Symbol, String}[]
        end
        ...
        push!(s.config[:error_history], (now(), ai, error_category, error))
        ...
        @debug "Error tracking updated" ai=ai error_category=error_category total_errors=sum(values(s.config[:error_stats]))
        
    catch e
        @error "Error updating error tracking" ai=ai error=e
    end
end

"""
    cancelorders!(s::SC, ai::AssetInstance; timeout::Period = Minute(2), reason::String = "timeout")
...
# Arguments
- `s::SC`: Strategy instance
- `ai::AssetInstance`: Asset instance to cancel orders for
...
"""
function cancelorders!(s::SC, ai::AssetInstance; timeout::Period = Minute(2), reason::String = "timeout")
    try
        @info "Canceling orders" ai=ai timeout=timeout reason=reason
        
        # Get list of open orders (in practice, this would query the exchange)
        open_orders = get_open_orders(s, ai)
        
        if isempty(open_orders)
            @debug "No open orders to cancel" ai=ai
            return 0
        end
        ...
        for order in open_orders
            try
                ...
                if should_cancel
                    success = cancel_single_order(s, ai, order, cancel_reason)
                    if success
                        canceled_count += 1
                        @info "Order canceled" ai=ai order_id=order[:order_id] reason=cancel_reason
                    else
                        @warn "Failed to cancel order" ai=ai order_id=order[:order_id]
                    end
                end
                
            catch e
                @error "Error processing order for cancellation" ai=ai order=order error=e
            end
        end
        
        # Update cancellation tracking
        update_cancellation_tracking(s, ai, canceled_count, reason)
        
        @info "Order cancellation completed" ai=ai canceled_count=canceled_count total_orders=length(open_orders)
        
        return canceled_count
        
    catch e
        @error "Error in order cancellation" ai=ai error=e
        return 0
    end
end

"""
    get_open_orders(s::SC, ai::AssetInstance)

Get list of open orders for an asset.
In practice, this would query the exchange API.
"""
function get_open_orders(s::SC, ai::AssetInstance)
    try
        # In a real implementation, this would query the exchange
        # For now, we'll simulate with stored pending orders
        
        pending_orders = get(s.config, :pending_orders, Dict{AssetInstance, Vector{Dict{Symbol, Any}}}())
        
        if haskey(pending_orders, ai)
            return pending_orders[ai]
        else
            return Dict{Symbol, Any}[]
        end
        
    catch e
        @error "Error getting open orders" ai=ai error=e
        return Dict{Symbol, Any}[]
    end
end

"""
    cancel_single_order(s::SC, ai::AssetInstance, order::Dict{Symbol, Any}, reason::String)

Cancel a single order with proper error handling.
"""
function cancel_single_order(s::SC, ai::AssetInstance, order::Dict{Symbol, Any}, reason::String)
    try
        order_id = get(order, :order_id, "unknown")
        
        @debug "Canceling single order" ai=ai order_id=order_id reason=reason
        ...
        if rand() < success_probability
            ...
            if haskey(s.config, :pending_orders) && haskey(s.config[:pending_orders], ai)
                filter!(o -> get(o, :order_id, "") != order_id, s.config[:pending_orders][ai])
            end
            
            return true
        else
            @warn "Order cancellation failed" ai=ai order_id=order_id
            return false
        end
        
    catch e
        @error "Error canceling single order" ai=ai order=order error=e
        return false
    end
end

"""
    update_cancellation_tracking(s::SC, ai::AssetInstance, canceled_count::Int, reason::String)

Update cancellation tracking statistics.
"""
function update_cancellation_tracking(s::SC, ai::AssetInstance, canceled_count::Int, reason::String)
    try
        ...
        @debug "Cancellation tracking updated" ai=ai canceled_count=canceled_count reason=reason total=s.config[:total_cancellations]
        
    catch e
        @error "Error updating cancellation tracking" ai=ai error=e
    end
end

"""
    check_posside(s::SC, ai::AssetInstance, intended_side::PositionSide)
...
"""
function check_posside(s::SC, ai::AssetInstance, intended_side::PositionSide)
    try
        @debug "Checking position side consistency" ai=ai intended_side=intended_side
        
        # Get current position information
        has_position = haspositions(s, ai)
        current_position = has_position ? position(s, ai) : 0.0
        ...
        validation_results = analyze_position_side_consistency(s, ai, current_side, intended_side)
        
        # Check for potential issues
        issues = check_position_side_issues(s, ai, current_side, intended_side)
        
        # Generate recommendations
        recommendations = generate_position_side_recommendations(s, ai, current_side, intended_side, issues)
        ...
        if status == :invalid
            @warn "Position side validation failed" ai=ai result=result
        elseif status == :warning
            @info "Position side validation warnings" ai=ai issues=length(issues)
        else
            @debug "Position side validation passed" ai=ai
        end
        
        return result
        
    catch e
        @error "Error checking position side" ai=ai intended_side=intended_side error=e
        return (
            status = :error,
...
"""
    analyze_position_side_consistency(s::SC, ai::AssetInstance, current_side, intended_side::PositionSide)

Analyze the consistency between current and intended position sides.
"""
function analyze_position_side_consistency(s::SC, ai::AssetInstance, current_side, intended_side::PositionSide)
    try
        ...
        @error "Error analyzing position side consistency" ai=ai error=e
        return (
            is_consistent = false,
            is_reversal = false,
            is_increase = false,
...
"""
    check_position_side_issues(s::SC, ai::AssetInstance, current_side, intended_side::PositionSide)

Check for potential issues with the intended position side.
"""
function check_position_side_issues(s::SC, ai::AssetInstance, current_side, intended_side::PositionSide)
    issues = []
    
    try
        # Check for rapid position reversals
        if current_side !== nothing && typeof(current_side) != typeof(intended_side)
            last_reversal_time = get(s.config, :last_reversal_time, Dict{AssetInstance, DateTime}())
            
            if haskey(last_reversal_time, ai)
                time_since_reversal = now() - last_reversal_time[ai]
        ...
        position_changes = get(s.config, :position_changes, Dict{AssetInstance, Int}())
        changes_today = get(position_changes, ai, 0)
        ...
        if haskey(s.config, :recent_signals)
            ...
            if haskey(recent_signals, ai)
                last_signal = recent_signals[ai]
        ...
        @error "Error checking position side issues" ai=ai error=e
        push!(issues, (
            severity = :high,
            type = :check_error,
...
"""
    generate_position_side_recommendations(s::SC, ai::AssetInstance, current_side, intended_side::PositionSide, issues)

Generate recommendations based on position side analysis.
"""
function generate_position_side_recommendations(s::SC, ai::AssetInstance, current_side, intended_side::PositionSide, issues)
    ...
    try
        ...
        @error "Error generating position side recommendations" ai=ai error=e
...
"""
    send_error_notification(s::SC, ai::AssetInstance, error::String, order_params::Dict{Symbol, Any})

Send error notification through configured channels (e.g., Telegram).
"""
function send_error_notification(s::SC, ai::AssetInstance, error::String, order_params::Dict{Symbol, Any})
    try
        ...
        @error "Error sending error notification" ai=ai error=e
    end
end