...
struct MockStrategy
    ...
    MockStrategy() = new(Dict{Symbol, Any}(), Dict{MockAssetInstance, MockOHLCV}(), Dict{Symbol, Any}())
end
...
# Mock functions
haspositions(s::MockStrategy, ai::MockAssetInstance) = haskey(s.config, :positions) && haskey(s.config[:positions], ai)
position(s::MockStrategy, ai::MockAssetInstance) = get(get(s.config, :positions, Dict()), ai, 0.0)
cash(s::MockStrategy, ai::MockAssetInstance) = get(get(s.config, :cash, Dict()), ai, 10000.0)
get_min_position_size(s::MockStrategy, ai::MockAssetInstance) = 10.0
get_max_position_size(s::MockStrategy, ai::MockAssetInstance) = 5000.0
...
@testset "Risk Management Tests" begin
    ...
    @testset "closeposition! function" begin
        ...
        s.universe[ai] = MockOHLCV([50000.0, 50100.0, 49900.0, 50200.0])
        ...
        result = closeposition!(s, ai; reason="test_close")
        @test result == true
        
        # Check tracking was updated
        @test haskey(s.config, :last_close_time)
        @test haskey(s.config[:last_close_time], ai)
        ...
        s.config[:positions][ai] = -0.05  # Short position
        
        result = closeposition!(s, ai; reason="emergency_test", emergency=true)
        @test result == true
        @test s.config[:close_reasons]["emergency_test"] == 1
        @test s.config[:total_closes] == 2
        
        # Test closing very small position
        s.config[:positions][ai] = 0.0001  # Very small position
        
        result = closeposition!(s, ai; reason="small_position")
    ...
    @testset "manage_cash_reserves function" begin
        ...
        s.universe[ai1] = MockOHLCV([50000.0])
        s.universe[ai2] = MockOHLCV([3000.0])
        s.config[:cash] = Dict(ai1 => 8000.0, ai2 => 2000.0)  # Total: 10000
        ...
        s.universe[ai1] = MockOHLCV([50000.0, 52000.0, 48000.0, 51000.0, 47000.0, 53000.0] .+ randn(6) .* 1000)
        s.universe[ai2] = MockOHLCV([3000.0, 3200.0, 2800.0, 3100.0, 2900.0, 3300.0] .+ randn(6) .* 100)
    ...
    @testset "calculate_volatility_reserve_multiplier function" begin
        ...
        s.universe[ai] = MockOHLCV(low_vol_prices)
        ...
        s.universe[ai] = MockOHLCV(high_vol_prices)
        ...
        s.universe[ai] = MockOHLCV([50000.0, 50100.0])  # Only 2 data points
    ...
    @testset "manage_collateral function" begin
        ...
        s.config[:positions] = Dict(ai => 0.2)  # 0.2 BTC position
        
        collateral_leveraged = manage_collateral(s, ai)
        ...
        s.config[:cash] = Dict(ai => 6000.0)  # Reduced cash
        
        collateral_critical = manage_collateral(s, ai)
        ...
        collateral_warning = manage_collateral(s, ai)
    ...
    @testset "peak_cash! function" begin
        ...
        s.universe[ai1] = MockOHLCV([50000.0])
        s.universe[ai2] = MockOHLCV([3000.0])
        s.config[:cash] = Dict(ai1 => 8000.0, ai2 => 2000.0)
        ...
        s.config[:cash] = Dict(ai1 => 9000.0, ai2 => 3000.0)
        ...
        for i in 1:1100  # Add more than 1000 entries
            s.config[:cash] = Dict(ai1 => 8000.0 + i, ai2 => 2000.0)
            peak_cash!(s)
        end
    ...
    @testset "calculate_drawdown function" begin
        ...
        s_no_peak.universe[ai] = MockOHLCV([50000.0])
        s_no_peak.config[:cash] = Dict(ai => 10000.0)
    ...
    @testset "check_risk_limits function" begin
        ...
        s.universe[ai] = MockOHLCV([50000.0])
        s.config[:cash] = Dict(ai => 10000.0)
        s.config[:positions] = Dict(ai => 0.05)  # Small position
        
        # Mock closeposition! function for testing
        closeposition!(s::MockStrategy, ai::MockAssetInstance; reason::String="", emergency::Bool=false) = true
        
        # Test normal conditions (no violations)
        risk_check = check_risk_limits(s, ai)
        ...
        s.config[:positions] = Dict(ai => 0.15)  # Large position (0.15 * 50000 = 7500 > 5000 limit)
        
        risk_check_size = check_risk_limits(s, ai)
        ...
        s.config[:positions] = Dict(ai => 0.2)  # Large leveraged position
        s.config[:cash] = Dict(ai => 1500.0)  # Low cash for collateral
        
        risk_check_collateral = check_risk_limits(s, ai)
        ...
        s.config[:cash] = Dict(ai => 10000.0)  # Reset cash
        s.config[:positions] = Dict(ai => 0.08)  # Position value: 0.08 * 50000 = 4000
        s.config[:max_concentration] = 0.30  # 30% limit
        # Concentration: 4000 / 10000 = 40% > 30% limit
        
        risk_check_concentration = check_risk_limits(s, ai)
    ...
    @testset "Helper functions" begin
        ...
        tick_size_custom = get_tick_size(s, ai)
    ...
    @testset "Error handling and edge cases" begin
        ...
        collateral_info = manage_collateral(s_empty, ai)
        ...
        s_corrupt.universe[ai] = MockOHLCV(Float64[])  # Empty price data
        s_corrupt.config[:cash] = Dict(ai => -1000.0)  # Negative cash
        
        peak = peak_cash!(s_corrupt)
        @test peak >= 0.0  # Should handle gracefully
        
        # Test risk limits with no position
        risk_check_no_pos = check_risk_limits(s_empty, ai)
        @test risk_check_no_pos.risk_level in [:normal, :error]
        
        # Test closeposition! with errors
        s_error = MockStrategy()
        s_error.universe[ai] = MockOHLCV([50000.0])
        s_error.config[:positions] = Dict(ai => 0.1)
        ...
        result = closeposition!(s_error, ai; reason="error_test")
