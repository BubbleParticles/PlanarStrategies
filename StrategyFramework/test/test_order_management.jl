...
mutable struct MockOrderStrategy
    ...
    balance::Float64
    leverage_settings::Dict{MockInstrumentInstance, Float64}
    ...
    function MockOrderStrategy()
        ...
        new(config, MockOrder[], Dict{MockInstrumentInstance, Float64}(), 10000.0, Dict{MockInstrumentInstance, Float64}())
    end
end

# Mock functions
isopen(ii::MockInstrumentInstance) = true
isopen(ii::MockInstrumentInstance, ps::MockPositionSide) = true
posside(order::MockOrder) = MockLong()
orderside(order::MockOrder) = order.side
closeat(ii::MockInstrumentInstance, ats) = 50000.0
...
@testset "Order Management Tests" begin
    ...
    @testset "trade! function" begin
        ...
        function trade!(s::MockOrderStrategy, ii::MockInstrumentInstance, ats, ts; 
                       pos::MockPositionSide, side::MockOrderSide, amount::Float64, 
                       t::Symbol = :gtc, kwargs...)
            
            # Validate amount
            if amount < ii.limits.amount.min
                @warn "Amount below minimum" amount ii.limits.amount.min
                return nothing
            end
            
            if amount > ii.limits.amount.max
                @warn "Amount above maximum" amount ii.limits.amount.max
                return nothing
            end
            
            # Create order
            price = closeat(ii, ats)
            leverage = get(s.leverage_settings, ii, s.config.def_lev)
            
            order = MockOrder(ii, side, amount, price, leverage)
        ...
        order = trade!(s, ii, ats, ts; pos = MockLong(), side = MockBuy(), amount = 0.1)
    ...
    @testset "handle_order_error function" begin
        ...
        function handle_order_error(s::MockOrderStrategy, ii::MockInstrumentInstance, order::MockOrder, error::MockOrderError)
            ...
            elseif error isa MockInsufficientFunds
                ...
                if new_amount >= ii.limits.amount.min
        ...
        order1 = MockOrder(ii, MockBuy(), 0.1, 50000.0)
        result1 = handle_order_error(s, ii, order1, MockOrderCanceled())
        @test result1 == :cancelled
        @test order1.status == :cancelled
        
        # Test insufficient funds with retry
        order2 = MockOrder(ii, MockBuy(), 0.1, 50000.0)
        result2 = handle_order_error(s, ii, order2, MockInsufficientFunds())
        @test result2 == :retry
        @test order2.status == :retry
        @test order2.amount == 0.05  # Half the original amount
        
        # Test insufficient funds with amount too small
        order3 = MockOrder(ii, MockBuy(), 0.0005, 50000.0)  # Below minimum after halving
        result3 = handle_order_error(s, ii, order3, MockInsufficientFunds())
        @test result3 == :failed
        @test order3.status == :failed
        
        # Test invalid price adjustment
        order4 = MockOrder(ii, MockBuy(), 0.1, 1000.0)  # Wrong price
        result4 = handle_order_error(s, ii, order4, MockInvalidPrice())
        @test result4 == :retry
        ...
        order5 = MockOrder(ii, MockBuy(), 0.1, 50000.0)
        result5 = handle_order_error(s, ii, order5, MockUnknownError())
        @test result5 == :failed
    ...
    @testset "cancelorders! function" begin
        ...
        # Add some orders
        order1 = MockOrder(ii, MockBuy(), 0.1, 50000.0)
        order2 = MockOrder(ii, MockSell(), 0.05, 51000.0)
        order3 = MockOrder(ii, MockBuy(), 0.2, 49000.0)
        ...
        function cancelorders!(s::MockOrderStrategy, ii::MockInstrumentInstance; side = nothing)
            cancelled_count = 0
            for order in s.orders
                if order.asset == ii && order.status == :pending
        ...
        result_buy = cancelorders!(s, ii; side = MockBuy())
        ...
        result_empty = cancelorders!(empty_strategy, ii)
    ...
    @testset "check_posside function" begin
        ...
        function check_posside(s::MockOrderStrategy, ii::MockInstrumentInstance, ats; 
        ...
        order = MockOrder(ii, MockBuy(), 0.1, 50000.0)
        result1 = check_posside(s, ii, now(); order_type = MockGTCOrder(), trade_result = order)
        @test result1 == true
        
        # Test with pending orders
        push!(s.orders, order)
        order.status = :pending
        result2 = check_posside(s, ii, now(); order_type = MockGTCOrder(), trade_result = nothing)
        @test result2 == true
        
        # Test with market order (should always pass)
        result3 = check_posside(s, ii, now(); order_type = MockMarketOrder(), trade_result = order)
        @test result3 == true
    end
    ...
    @testset "Order validation functions" begin
        ...
        function validate_trade_parameters(s::MockOrderStrategy, ii::MockInstrumentInstance, 
                                         amount::Float64, price::Float64)
            errors = String[]
            
            if amount < ii.limits.amount.min
                push!(errors, "Amount below minimum: $(amount) < $(ii.limits.amount.min)")
            end
            
            if amount > ii.limits.amount.max
                push!(errors, "Amount above maximum: $(amount) > $(ii.limits.amount.max)")
            end
            
            if price < ii.limits.price.min
                push!(errors, "Price below minimum: $(price) < $(ii.limits.price.min)")
            end
            
            if price > ii.limits.price.max
                push!(errors, "Price above maximum: $(price) > $(ii.limits.price.max)")
            end
    ...
    @testset "Leverage adjustment" begin
        ...
        function calculate_leverage_adjustment(s::MockOrderStrategy, ii::MockInstrumentInstance, 
        ...
        lev1 = calculate_leverage_adjustment(s, ii, 1.0, market_normal)
        @test lev1 == 1.0
        
        # Test high volatility
        market_high_vol = Dict(:volatility => 0.06, :trend_strength => 0.5)
        lev2 = calculate_leverage_adjustment(s, ii, 1.0, market_high_vol)
        @test lev2 < 1.0
        
        # Test strong trend
        market_strong_trend = Dict(:volatility => 0.02, :trend_strength => 0.9)
        lev3 = calculate_leverage_adjustment(s, ii, 1.0, market_strong_trend)
        @test lev3 > 1.0
        
        # Test extreme values
        market_extreme = Dict(:volatility => 0.1, :trend_strength => 1.0)
        lev4 = calculate_leverage_adjustment(s, ii, 10.0, market_extreme)
        @test lev4 <= 5.0  # Should be clamped
        
        # Test minimum clamp
        market_extreme_low = Dict(:volatility => 0.1, :trend_strength => 0.1)
        lev5 = calculate_leverage_adjustment(s, ii, 0.05, market_extreme_low)
    ...
    @testset "Order timeout handling" begin
        ...
        old_order = MockOrder(ii, MockBuy(), 0.1, 50000.0)
        old_order.timestamp = now() - Minute(5)  # 5 minutes old
        old_order.status = :pending
        
        recent_order = MockOrder(ii, MockSell(), 0.05, 51000.0)
