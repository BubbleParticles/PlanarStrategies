...
function should_market_make(s::MockStrategy, ai::MockAssetInstance, ats::DateTime)
    try
...
function get_make_amounts(s::MockStrategy, ai::MockAssetInstance, max_position_pct::Float64)
    try
...
function calculate_optimal_spread(s::MockStrategy, ai::MockAssetInstance, target_spread_pct::Float64, market_conditions)
    try
...
function analyze_market_making_conditions(s::MockStrategy, ai::MockAssetInstance)
    try
        ohlcv = s.universe[ai]
...
@testset "Market Making Standalone Tests" begin
    ...
    @testset "get_make_amounts" begin
        ...
        amounts = get_make_amounts(s, ai, 0.1)
