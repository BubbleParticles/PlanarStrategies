...
mutable struct MockSignalStrategy
    config::NamedTuple
    signal_states::Dict{MockInstrumentInstance, Any}
    market_data::Dict{MockInstrumentInstance, Dict{Symbol, Float64}}
...
function generate_buy_signal(sg::SimpleBuySignalGenerator, s::MockSignalStrategy, ii::MockInstrumentInstance, ats)
    # Simple mock: return buy signal based on threshold and random factor
    market_factor = get(get(s.market_data, ii, Dict()), :momentum, 0.5)
    return market_factor > sg.buy_threshold ? 0.8 : 0.2
end

function generate_sell_signal(sg::SimpleBuySignalGenerator, s::MockSignalStrategy, ii::MockInstrumentInstance, ats)
    # Simple mock: return sell signal based on threshold
    market_factor = get(get(s.market_data, ii, Dict()), :momentum, 0.5)
    return market_factor < sg.sell_threshold ? 0.8 : 0.2
end

function generate_buy_signal(sg::AlwaysBuySignalGenerator, s::MockSignalStrategy, ii::MockInstrumentInstance, ats)
    return 1.0  # Always buy
end

function generate_sell_signal(sg::AlwaysBuySignalGenerator, s::MockSignalStrategy, ii::MockInstrumentInstance, ats)
    return 0.0  # Never sell
end

function generate_buy_signal(sg::NeverTradeSignalGenerator, s::MockSignalStrategy, ii::MockInstrumentInstance, ats)
    return 0.0  # Never buy
end

function generate_sell_signal(sg::NeverTradeSignalGenerator, s::MockSignalStrategy, ii::MockInstrumentInstance, ats)
    return 0.0  # Never sell
end

function generate_buy_signal(sg::RSISignalGenerator, s::MockSignalStrategy, ii::MockInstrumentInstance, ats)
    rsi = get(get(s.market_data, ii, Dict()), :rsi, 50.0)
    return rsi < sg.rsi_oversold ? 0.9 : 0.1
end

function generate_sell_signal(sg::RSISignalGenerator, s::MockSignalStrategy, ii::MockInstrumentInstance, ats)
    rsi = get(get(s.market_data, ii, Dict()), :rsi, 50.0)
    return rsi > sg.rsi_overbought ? 0.9 : 0.1
end

function generate_buy_signal(sg::MovingAverageCrossoverGenerator, s::MockSignalStrategy, ii::MockInstrumentInstance, ats)
    fast_ma = get(get(s.market_data, ii, Dict()), :fast_ma, 50000.0)
    slow_ma = get(get(s.market_data, ii, Dict()), :slow_ma, 50000.0)
    return fast_ma > slow_ma ? 0.8 : 0.2
end

function generate_sell_signal(sg::MovingAverageCrossoverGenerator, s::MockSignalStrategy, ii::MockInstrumentInstance, ats)
    fast_ma = get(get(s.market_data, ii, Dict()), :fast_ma, 50000.0)
    slow_ma = get(get(s.market_data, ii, Dict()), :slow_ma, 50000.0)
    return fast_ma < slow_ma ? 0.8 : 0.2
end
...
function should_trade(sg::SignalGenerator, s::MockSignalStrategy, ii::MockInstrumentInstance, ats)
    return true  # Default: always allow trading
end

function should_trade(sg::NeverTradeSignalGenerator, s::MockSignalStrategy, ii::MockInstrumentInstance, ats)
...
@testset "Signal Interface Tests" begin
    ...
    @testset "SignalGenerator interface compliance" begin
        ...
        for sg in generators
            ...
            @test should_trade(sg, s, ii, ats) isa Bool
    ...
    @testset "SimpleBuySignalGenerator behavior" begin
        ...
        s.market_data[ii] = Dict(:momentum => 0.8)
        buy_signal_high = generate_buy_signal(sg, s, ii, ats)
        sell_signal_high = generate_sell_signal(sg, s, ii, ats)
        ...
        s.market_data[ii] = Dict(:momentum => 0.2)
        buy_signal_low = generate_buy_signal(sg, s, ii, ats)
        sell_signal_low = generate_sell_signal(sg, s, ii, ats)
        ...
        @test should_trade(sg, s, ii, ats) == true
    end
    ...
    @testset "NeverTradeSignalGenerator behavior" begin
        ...
        for i in 1:10
            ...
            @test buy_signal == 0.0
            @test sell_signal == 0.0
            @test should_trade(sg, s, ii, ats) == false
        end
    end
    ...
    @testset "RSISignalGenerator behavior" begin
        ...
        s.market_data[ii] = Dict(:rsi => 25.0)  # Below oversold threshold
        buy_signal_oversold = generate_buy_signal(sg, s, ii, ats)
        sell_signal_oversold = generate_sell_signal(sg, s, ii, ats)
        ...
        s.market_data[ii] = Dict(:rsi => 75.0)  # Above overbought threshold
        buy_signal_overbought = generate_buy_signal(sg, s, ii, ats)
        sell_signal_overbought = generate_sell_signal(sg, s, ii, ats)
        
        @test buy_signal_overbought < 0.2  # Weak buy signal
        @test sell_signal_overbought > 0.8 # Strong sell signal
        
        # Test neutral condition
        s.market_data[ii] = Dict(:rsi => 50.0)  # Neutral RSI
        buy_signal_neutral = generate_buy_signal(sg, s, ii, ats)
        sell_signal_neutral = generate_sell_signal(sg, s, ii, ats)
        
        @test buy_signal_neutral < 0.2   # Weak buy signal
        @test sell_signal_neutral < 0.2  # Weak sell signal
        
        # Test edge cases
        s.market_data[ii] = Dict(:rsi => 30.0)  # Exactly at oversold threshold
        buy_signal_edge = generate_buy_signal(sg, s, ii, ats)
        @test buy_signal_edge < 0.2  # Should not trigger (not below threshold)
        
        s.market_data[ii] = Dict(:rsi => 70.0)  # Exactly at overbought threshold
        sell_signal_edge = generate_sell_signal(sg, s, ii, ats)
    ...
    @testset "MovingAverageCrossoverGenerator behavior" begin
        ...
        s.market_data[ii] = Dict(:fast_ma => 51000.0, :slow_ma => 50000.0)
        buy_signal_bullish = generate_buy_signal(sg, s, ii, ats)
        sell_signal_bullish = generate_sell_signal(sg, s, ii, ats)
        ...
        s.market_data[ii] = Dict(:fast_ma => 49000.0, :slow_ma => 50000.0)
        buy_signal_bearish = generate_buy_signal(sg, s, ii, ats)
        sell_signal_bearish = generate_sell_signal(sg, s, ii, ats)
        ...
        s.market_data[ii] = Dict(:fast_ma => 50000.0, :slow_ma => 50000.0)
        buy_signal_equal = generate_buy_signal(sg, s, ii, ats)
        sell_signal_equal = generate_sell_signal(sg, s, ii, ats)
    ...
    @testset "Signal state management" begin
        ...
        function initialize_signal_state!(s::MockSignalStrategy, ii::MockInstrumentInstance, lifetime::Float64 = 0.2)
            s.signal_states[ii] = MockSignalState(lifetime)
            return true
        end
        
        function update_signal_state!(s::MockSignalStrategy, ii::MockInstrumentInstance, 
                                    buy_signal::Float64, sell_signal::Float64, ats::DateTime)
            if !haskey(s.signal_states, ii)
                initialize_signal_state!(s, ii)
            end
            
            state = s.signal_states[ii]
        ...
        function is_signal_active(s::MockSignalStrategy, ii::MockInstrumentInstance, signal_type::Symbol)
            if !haskey(s.signal_states, ii)
                return false
            end
        ...
        @test initialize_signal_state!(s, ii, 0.3)
        @test haskey(s.signal_states, ii)
        @test s.signal_states[ii].signal_lifetime == 0.3
        @test !s.signal_states[ii].buy_signal_active
        @test !s.signal_states[ii].sell_signal_active
        
        # Test signal activation
        ats = now()
        state = update_signal_state!(s, ii, 0.8, 0.2, ats)  # Strong buy signal
        @test state.buy_signal_active == true
        @test state.sell_signal_active == false
        @test state.last_buy_signal == ats
        
        @test is_signal_active(s, ii, :buy) == true
        @test is_signal_active(s, ii, :sell) == false
        
        # Test sell signal activation
        state = update_signal_state!(s, ii, 0.2, 0.9, ats + Second(1))  # Strong sell signal
        @test state.sell_signal_active == true
        @test is_signal_active(s, ii, :sell) == true
        ...
        update_signal_state!(s, ii, 0.1, 0.1, ats)  # Weak signals, should expire old ones
    ...
    @testset "Signal validation and debugging" begin
        ...
        function debug_signal_generation(sg::SignalGenerator, s::MockSignalStrategy, 
                                       ii::MockInstrumentInstance, ats::DateTime)
            ...
            # Additional context
            debug_info["should_trade"] = should_trade(sg, s, ii, ats)
            debug_info["signal_lifetime"] = get_signal_lifetime(sg)
            debug_info["market_data"] = get(s.market_data, ii, Dict())
        ...
        debug_info = debug_signal_generation(sg, s, ii, now())
    ...
    @testset "Signal generator composition and chaining" begin
        ...
        function generate_buy_signal(sg::CompositeSignalGenerator, s::MockSignalStrategy, ii::MockInstrumentInstance, ats)
            weighted_signal = 0.0
            for (gen, weight) in zip(sg.generators, sg.weights)
                signal = generate_buy_signal(gen, s, ii, ats)
                weighted_signal += signal * weight
            end
            return clamp(weighted_signal, 0.0, 1.0)
        end
        
        function generate_sell_signal(sg::CompositeSignalGenerator, s::MockSignalStrategy, ii::MockInstrumentInstance, ats)
            weighted_signal = 0.0
            for (gen, weight) in zip(sg.generators, sg.weights)
                signal = generate_sell_signal(gen, s, ii, ats)
                weighted_signal += signal * weight
            end
            return clamp(weighted_signal, 0.0, 1.0)
        end
        
        function should_trade(sg::CompositeSignalGenerator, s::MockSignalStrategy, ii::MockInstrumentInstance, ats)
            # All generators must allow trading
            return all(should_trade(gen, s, ii, ats) for gen in sg.generators)
        end
        ...
        s.market_data[ii] = Dict(:momentum => 0.8, :rsi => 25.0)  # Bullish conditions
        
        buy_signal = generate_buy_signal(composite_sg, s, ii, ats)
        sell_signal = generate_sell_signal(composite_sg, s, ii, ats)
        
        @test 0.0 <= buy_signal <= 1.0
        @test 0.0 <= sell_signal <= 1.0
        @test should_trade(composite_sg, s, ii, ats) == true
        ...
        @test should_trade(composite_never, s, ii, ats) == false  # Should be false due to NeverTradeSignalGenerator
    end
end
